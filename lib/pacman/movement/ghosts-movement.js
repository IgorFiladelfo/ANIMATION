...
const SCATTER_CORNERS = {
    blinky: { x: constants_1.GRID_WIDTH - 3, y: 0 }, // Top right corner
    pinky: { x: 0, y: 0 }, // Top left corner
    inky: { x: constants_1.GRID_WIDTH - 3, y: constants_1.GRID_HEIGHT - 1 }, // Bottom right corner
    clyde: { x: 0, y: constants_1.GRID_HEIGHT - 1 } // Bottom left corner
};
...
const moveGhostToScatterTarget = (ghost, store) => {
    ...
    if (ghost.x === target.x && ghost.y === target.y) {
        const moves = movement_utils_1.MovementUtils.getValidMoves(ghost.x, ghost.y);
        if (moves.length > 0) {
            const [dx, dy] = moves[0];
            ghost.x += dx;
            ghost.y += dy;
...
const getValidMovesWithoutReverse = (ghost) => {
    const validMoves = movement_utils_1.MovementUtils.getValidMoves(ghost.x, ghost.y);
...
const moveEyesToHome = (ghost, store) => {
    ...
    const nextMove = movement_utils_1.MovementUtils.findNextStepDijkstra({ x: ghost.x, y: ghost.y }, respawnPosition);
    if (nextMove) {
...
const BFSTargetLocation = (startX, startY, targetX, targetY, currentDirection) => {
    ...
    if (startX === targetX && startY === targetY)
        return null;
    const queue = [{ x: startX, y: startY, direction: currentDirection || 'right' }];
    let queueHead = 0;
    ...
    while (queueHead < queue.length) {
        ...
        // Get valid moves
        const validMoves = movement_utils_1.MovementUtils.getValidMoves(x, y);
    ...
    const validMoves = movement_utils_1.MovementUtils.getValidMoves(startX, startY);
    if (validMoves.length > 0) {
...
const calculateGhostTarget = (ghost, store) => {
    ...
    if (ghost.name === 'blinky') {
        ...
        const totalDots = constants_1.GRID_WIDTH * constants_1.GRID_HEIGHT;
    ...
    switch (ghost.name) {
        ...
        case 'pinky': // Pink - tries to ambush Pac-Man by positioning herself in front of him
            ...
            targetX = Math.min(Math.max(targetX, 0), constants_1.GRID_WIDTH - 1);
            targetY = Math.min(Math.max(targetY, 0), constants_1.GRID_HEIGHT - 1);
        ...
        case 'inky': // Blue - Coordinated behavior with Blinky
            ...
            // Ensure the target is within the grid
            twoAhead.x = Math.min(Math.max(twoAhead.x, 0), constants_1.GRID_WIDTH - 1);
            twoAhead.y = Math.min(Math.max(twoAhead.y, 0), constants_1.GRID_HEIGHT - 1);
            return twoAhead;
        case 'clyde': // Orange - Toggles between chasing and random
            const distanceToPacman = movement_utils_1.MovementUtils.calculateDistance(ghost.x, ghost.y, pacman.x, pacman.y);
...
const getRandomDestination = (x, y) => {
    ...
    return {
        x: Math.max(0, Math.min(randomX, constants_1.GRID_WIDTH - 1)),
        y: Math.max(0, Math.min(randomY, constants_1.GRID_HEIGHT - 1))
    };
};
exports.GhostsMovement = {
    moveGhosts,
    resetGameMode
};
