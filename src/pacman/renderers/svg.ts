...
const GRID_STEP = CELL_SIZE + GAP_SIZE;

const generateAnimatedSVG = (store: StoreType) => {
	const gridPixelWidth = GRID_WIDTH * GRID_STEP;
	const gridPixelHeight = GRID_HEIGHT * GRID_STEP;
	...
	svg += `<metadata><info><frames>${store.gameHistory.length}</frames><frameRate>${1000 / DELTA_TIME}</frameRate><durationMs>${totalDurationMs}</durationMs><generatedOn>${new Date().toISOString()}</generatedOn></info></metadata>`;
	...
	if (store.ghosts.length > 0) svg += generateGhostsPredefinition();
	...
	if (store.config.showMonthLabels !== false) {
		for (let x = 0; x < GRID_WIDTH; x++) {
			if (store.monthLabels[x] && store.monthLabels[x] !== lastMonth) {
				const xPos = GRID_OFFSET_X + x * GRID_STEP + CELL_SIZE / 2;
	...
	for (let x = 0; x < GRID_WIDTH; x++) {
		for (let y = 0; y < GRID_HEIGHT; y++) {
			...
			svg += `<rect id="c-${x}-${y}" x="${cellX}" y="${cellY}" width="${CELL_SIZE}" height="${CELL_SIZE}" rx="3.2" fill="url(#cellGlow)" stroke="#15304a" stroke-width="0.75"/>`;
			...
			if (visual.hasPellet) {
				...
				if (visual.animated) svg += `<animate attributeName="opacity" dur="${animationDurationMs}ms" repeatCount="indefinite" calcMode="discrete" values="${visual.pelletOpacity.join(';')}" keyTimes="${cellColorAnimation.keyTimes}"/>`;
				svg += `</circle>`;
			}

			if (visual.hasBlock) {
				svg += `<rect x="${cellX + 2}" y="${cellY + 2}" width="${CELL_SIZE - 4}" height="${CELL_SIZE - 4}" rx="3" fill="${visual.blockFill[0]}" stroke="#8affb1" stroke-width="0.65" opacity="${visual.blockOpacity[0]}" filter="url(#blockGlow)">`;
				if (visual.animated) {
	...
	for (let y = 0; y < GRID_HEIGHT; y++) {
		...
		for (let x = 0; x <= GRID_WIDTH; x++) {
			const active = x < GRID_WIDTH && WALLS.horizontal[x][y].active;
			if (active && runStart === null) runStart = x;
			if ((!active || x === GRID_WIDTH) && runStart !== null) {
				const length = x - runStart;
				const wx = GRID_OFFSET_X + runStart * GRID_STEP - GAP_SIZE;
				const wy = GRID_OFFSET_Y + y * GRID_STEP - GAP_SIZE;
	...
	for (let x = 0; x < GRID_WIDTH; x++) {
		let runStart: number | null = null;
		for (let y = 0; y <= GRID_HEIGHT; y++) {
			const active = y < GRID_HEIGHT && WALLS.vertical[x][y].active;
			if (active && runStart === null) runStart = y;
			if ((!active || y === GRID_HEIGHT) && runStart !== null) {
				const length = y - runStart;
				const wx = GRID_OFFSET_X + x * GRID_STEP - GAP_SIZE;
				const wy = GRID_OFFSET_Y + runStart * GRID_STEP - GAP_SIZE;
	...
	svg += `<path id="pacman" d="${generatePacManPath(0.58)}" fill="${PACMAN_COLOR}" stroke="#ffe56c" stroke-width="0.7" filter="url(#pacGlow)">
	...
	store.ghosts.forEach((ghost, index) => {
		...
		for (const [state, keyframes] of Object.entries(stateChanges)) {
			...
			svg += `<use href="#ghost-${state}" width="${CELL_SIZE}" height="${CELL_SIZE}" visibility="${initialVisibility}"><animate attributeName="visibility" dur="${animationDurationMs}ms" repeatCount="indefinite" keyTimes="${keyTimes}" values="${values}"/></use>`;
		}
		svg += `</g>`;
	});
...
type ContributionVisualAnimation = {
	animated: boolean;
	hasPellet: boolean;
	hasBlock: boolean;
	pelletOpacity: string[];
	blockOpacity: string[];
	blockFill: string[];
};

const getContributionVisualAnimationData = (animation: AnimationData, palette: string[]): ContributionVisualAnimation => {
...
const generatePacManPosition = (pacman: StoreType['pacman']): string => {
	const x = GRID_OFFSET_X + pacman.x * (CELL_SIZE + GAP_SIZE);
	const y = GRID_OFFSET_Y + pacman.y * (CELL_SIZE + GAP_SIZE);
	return `${x},${y}`;
};
...
const generateGhostPosition = (ghost: StoreType['ghosts'][number]): string => {
	...
	const x = GRID_OFFSET_X + fx * (CELL_SIZE + GAP_SIZE);
	const y = GRID_OFFSET_Y + fy * (CELL_SIZE + GAP_SIZE);
	return `${x},${y}`;
};
...
const generateGhostsPredefinition = () => {
		...
		['up', 'down', 'left', 'right'].forEach((direction) => {
			...
			if (direction in ghostObj) {
                ...
                <symbol id="ghost-${ghostName}-${direction}" viewBox="0 0 ${CELL_SIZE} ${CELL_SIZE}">
                    <image href="${ghostObj[direction]}" width="${CELL_SIZE}" height="${CELL_SIZE}" style="image-rendering:pixelated"/>
                </symbol>
                `;
			}
		});
    ...
    <symbol id="ghost-scared" viewBox="0 0 ${CELL_SIZE} ${CELL_SIZE}">
        <image href="${(GHOSTS['scared'] as { imgDate: string }).imgDate}" width="${CELL_SIZE}" height="${CELL_SIZE}" style="image-rendering:pixelated"/>
    </symbol>`;
	...
	['up', 'down', 'left', 'right'].forEach((direction) => {
		if (GHOSTS['eyes'] && direction in (GHOSTS['eyes'] as Record<string, string>)) {
            ...
            <symbol id="ghost-eyes-${direction}" viewBox="0 0 ${CELL_SIZE} ${CELL_SIZE}">
                <image href="${eyesObj[direction]}" width="${CELL_SIZE}" height="${CELL_SIZE}" style="image-rendering:pixelated"/>
            </symbol>
            `;
		} else {
